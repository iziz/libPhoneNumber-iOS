...
+ (NSDictionary *)shortNumberDataMap {
    ...
    dispatch_once(&onceToken, ^{
        ...
        // See GeneratePhoneNumberHeader.sh and PhoneNumberMetaData.h for details.
        NSMutableData* gunzippedData =
            [NSMutableData dataWithLength:kShortNumberMetaDataExpandedLength];
        ...
        zStream.next_in = kShortNumberMetaData;
        zStream.avail_in = (uint)kShortNumberMetaDataCompressedLength;
        zStream.next_out = (Bytef *)gunzippedData.bytes;
        zStream.avail_out = (uint)gunzippedData.length;
        ...
        NSError *error = nil;
        shortNumberDataDictionary = [NSJSONSerialization JSONObjectWithData:gunzippedData
                                                                    options:0
                                                                      error:&error];
        NSAssert(error == nil, @"Unable to convert JSON - %@", error);
    });
