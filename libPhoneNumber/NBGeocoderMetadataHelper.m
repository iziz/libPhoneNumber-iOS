  ...
  sqlite3_prepare_v2(
      ...
      [[NSString
          stringWithFormat:
              ...
              @"length(?)), toSearch as (SELECT substr(?, 1, x) as indata FROM cnt) select "
              @"nationalnumber, description, length(nationalnumber) as natLength from "
              @"geocodingPairs%@ where NATIONALNUMBER in toSearch order by natLength desc limit 1",
              countryCode] UTF8String],
      -1, &self->selectStatement, NULL);
  return self;
}
  ...
  @autoreleasepool {
    ...
    if (![phoneNumber.countryCode isEqualToNumber:self.countryCode]) {
      ...
      sqlite3_prepare_v2(
          self->DB,
          [[NSString
              stringWithFormat:@"WITH RECURSIVE cnt(x) AS ( SELECT 1 UNION ALL SELECT x+1 FROM cnt "
                               @"LIMIT length(?)), toSearch as (SELECT substr(?, 1, x) as indata "
                               @"FROM cnt) select nationalnumber, description, "
                               @"length(nationalnumber) as natLength from geocodingPairs%@ where "
                               @"NATIONALNUMBER in toSearch order by natLength desc limit 1",
                               [self.countryCode stringValue]] UTF8String],
          -1, &self->selectStatement, NULL);
    ...
    if (self->DB == NULL) {
      sql_command_results =
          sqlite3_open_v2([self->databasePath UTF8String], &self -> DB, SQLITE_OPEN_READONLY, NULL);
