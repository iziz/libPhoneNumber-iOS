...
- The previous local metadata baseline was `v9.0.38`.
- Updated main phone-number metadata to `v9.0.39`; generated testing and short-number metadata were unchanged.
- Regenerated geocoding metadata from `v9.0.39`; only `en.db` changed.
- Regenerated carrier metadata from `v9.0.39`; the packed database has 31,056 prefix rows and 107 mobile portable regions.
- Regenerated timezone metadata from `v9.0.39`; the source remained unchanged at 3,290 prefix rows and 285 unique timezone IDs, while the bundle baseline ref advanced.
- Set the release version to `2.1.0` because the commits since `2.0.1` also add visionOS support and public Swift 6 concurrency APIs. These changes are documented in `CHANGELOG.md`.

### Upstream Logic And Resource Comparison

- Compared tracked Google files from `v9.0.38` (`796d807ebb61`) to `v9.0.39` (`670015c8aec4`). Nine files changed: generated JavaScript phone-number metadata, phone-number and short-number XML resources, five carrier resources, and one geocoding resource.
- Tracked JavaScript implementation, test, and public API files did not change. Upstream test and API parity checks passed, so no Objective-C logic or test port was needed for this update.
- Compared `v9.0.39` to Google `master` (`806ee32e8c8c`); zero tracked source or resource files differed.

### Validation

```bash
swift scripts/updateMetadata.swift v9.0.39 --dry-run --output-root .build/metadata-update/v9.0.39-dry-run
swift scripts/checkUpstreamSourceDrift.swift --current-ref v9.0.38 --upstream-ref v9.0.39 --output .build/upstream-source-drift-v9.0.38-to-v9.0.39
swift scripts/checkUpstreamTestParity.swift --upstream-ref v9.0.39
swift scripts/checkUpstreamAPIParity.swift --upstream-ref v9.0.39
swift scripts/updateMetadata.swift v9.0.39 --output-root .build/metadata-update/v9.0.39
swift scripts/updateProjectVersions.swift 2.1.0
swift scripts/checkVersionConsistency.swift
swift test --scratch-path .build/validation-2.0.2-default
LC_ALL=ko_KR.UTF-8 LANG=ko_KR.UTF-8 swift test --scratch-path .build/validation-2.0.2-ko
swift build -c release --scratch-path .build/validation-2.0.2-release
swift scripts/testXcodeSchemes.swift --destination 'id=04DC65D3-F0FF-465D-9CFB-2AC923C471FB' --derived-data-root /tmp/libphone-2.0.2-xcode
swift test --sanitize=thread --scratch-path .build/validation-2.1.0-tsan
swift scripts/checkUpstreamSourceDrift.swift --current-ref v9.0.39 --upstream-ref master --output .build/upstream-source-drift-v9.0.39-to-master
swift scripts/checkMetadataFreshness.swift --current-ref v9.0.39 --output .build/metadata-freshness-v9.0.39-final --fail-on-update
swift scripts/publishPodspecs.swift --lint
git diff --check
```

- Scheduled Upstream Drift run `35175467837` detected `v9.0.39`. Its metadata freshness job failed as intended for an available update; source drift and Google master parity jobs passed.
- The dry run and checked-in generation completed. All 34 generated geocoding databases matched the bundled databases by SHA-256.
- Upstream `v9.0.39` parity passed for 173 JavaScript tests and 66 public prototype methods against 181 local Objective-C tests and 93 selectors.
- Version consistency passed for `2.1.0`, and the final freshness check passed.
- SwiftPM tests passed in the default and Korean locales: 231 XCTest tests and 43 Swift Testing tests in each run. The release build and thread sanitizer test run passed.
- Xcode tests passed for `libPhoneNumber` (150), `libPhoneNumberGeocoding` (20), and `libPhoneNumberShortNumber` (31) on the iPhone 17 / iOS simulator.

## 2026-08-29: Google libphonenumber v9.0.38

### Scope

- Previous local main, testing, and short-number metadata matched Google libphonenumber `v9.0.37`.
- Updated main phone-number metadata to `v9.0.38`.
- Testing and short-number metadata were unchanged between `v9.0.37` and `v9.0.38`.
- Regenerated geocoding metadata from `v9.0.38`; `en.db`, `pt.db`, and `zh.db` changed.
- Regenerated carrier metadata from `v9.0.38`; packed metadata now has 31,046 prefix rows and 107 mobile portable regions.
- Regenerated timezone metadata from `v9.0.38`; source data remained unchanged at 3,290 prefix rows and 285 unique timezone IDs, while the bundle baseline ref advanced to `v9.0.38`.
- Updated project and podspec versions to `1.7.8`.
- Fixed the geocoding updater so using `--output` together with `--replace-bundle` updates both the review output and the checked-in bundle, and added regression coverage for that path.

### Upstream Logic And Resource Comparison

- Compared tracked Google source and resource paths from `v9.0.37` (`b232be1f0da1`) to `v9.0.38` (`796d807ebb61`).
- All 17 changed tracked files were phone-number, carrier, or geocoding metadata resources.
...
- A final comparison from `v9.0.38` to Google `master` (`4ad67e90c65e`) found zero tracked source or resource changes.

### Commands

```bash
swift scripts/updateMetadata.swift v9.0.38 --dry-run --output-root .build/metadata-update/v9.0.38-dry-run-codex
swift scripts/checkUpstreamSourceDrift.swift --current-ref v9.0.37 --upstream-ref v9.0.38 --output .build/upstream-source-drift-v9.0.37-to-v9.0.38-codex
swift scripts/checkUpstreamTestParity.swift --upstream-ref v9.0.38
swift scripts/checkUpstreamAPIParity.swift --upstream-ref v9.0.38
swift scripts/updateMetadata.swift v9.0.38 --output-root .build/metadata-update/v9.0.38-codex
swift scripts/updateProjectVersions.swift 1.7.8
swift scripts/updateMetadata.swift v9.0.38 --only geocoding --output-root .build/metadata-update/v9.0.38-codex --skip-freshness-check
swift scripts/checkVersionConsistency.swift
scripts/testGeocodingMetadataUpdater.sh
swift test --scratch-path .build/validation-1.7.8-default
LC_ALL=ko_KR.UTF-8 LANG=ko_KR.UTF-8 swift test --scratch-path .build/validation-1.7.8-ko
swift build -c release --scratch-path .build/validation-1.7.8-release
swift scripts/testXcodeSchemes.swift --destination 'id=C768167E-DCA0-42C0-A2C0-813F6189F418' --derived-data-root /tmp/libphonenumber-1.7.8-xcode
swift scripts/publishPodspecs.swift --lint
swift scripts/checkMetadataFreshness.swift --current-ref v9.0.38 --output .build/metadata-freshness-v9.0.38-final-codex --fail-on-update
...
swift scripts/checkUpstreamSourceDrift.swift --current-ref v9.0.38 --upstream-ref master --output .build/upstream-source-drift-v9.0.38-to-master-codex
git diff --check
```

### Results

- Scheduled Upstream Drift run `33229406804` detected Google libphonenumber `v9.0.38` as the latest upstream tag; only the metadata freshness job failed, while source drift and Google master parity jobs passed.
- Main phone-number metadata changed; testing and short-number metadata were unchanged.
- Metadata generation resolved `v9.0.38` to peeled commit `796d807ebb61c56103c3c4f85b260870b29b312b`.
- Carrier metadata increased from 31,038 to 31,046 prefix rows and retained 107 mobile portable regions.
- Geocoding regeneration produced 34 databases from 224 source resources; `en.db`, `pt.db`, and `zh.db` changed.
- Timezone source data was unchanged; the regenerated database records `upstream_ref` `v9.0.38` with 3,290 prefix rows and 285 unique timezone IDs.
- Geocoding review output and all 34 checked-in bundle databases have matching SHA-256 digests after the updater fix.
- Upstream `v9.0.38` and Google `master` JS test parity passed with 173 upstream JS tests and 181 local Objective-C tests.
- Upstream `v9.0.38` and Google `master` JS API parity passed with 66 upstream public prototype methods and 93 local Objective-C public selectors.
- Google `master` source/resource drift check passed with zero changed tracked files after `v9.0.38`.
- Version consistency passed for `1.7.8`.
- Geocoding metadata updater regression test passed.
...
## 2026-08-15: Google libphonenumber v9.0.37

### Scope

- Previous local main, testing, and short-number metadata matched Google libphonenumber `v9.0.36`.
- Updated main phone-number metadata to `v9.0.37`.
- Testing metadata was unchanged between `v9.0.36` and `v9.0.37`.
- Updated short-number metadata to `v9.0.37`.
- Regenerated geocoding metadata from `v9.0.37`; checked-in geocoding databases were unchanged.
- Regenerated carrier metadata from `v9.0.37`; packed metadata now has 31,038 prefix rows and 107 mobile portable regions.
- Regenerated timezone metadata from `v9.0.37`; packed metadata now has 3,290 prefix rows and 285 unique timezone IDs.
- Updated project and podspec versions to `1.7.7`.

### Upstream Logic And Resource Comparison

- Compared tracked Google source and resource paths from `v9.0.36` (`eba87f5b1f76`) to `v9.0.37` (`b232be1f0da1`).
- All 18 changed tracked files were phone-number, short-number, carrier, geocoding, or timezone metadata resources.
- The tracked JavaScript implementation, test, and public API files were unchanged, so no Objective-C logic or test port was required.
- A final comparison from `v9.0.37` to Google `master` (`17c9061a3af1`) found zero tracked source or resource changes.

### Commands

```bash
swift scripts/updateMetadata.swift v9.0.37 --dry-run --output-root .build/metadata-update/v9.0.37-dry-run-codex
swift scripts/checkUpstreamSourceDrift.swift --current-ref v9.0.36 --upstream-ref v9.0.37 --output .build/upstream-source-drift-v9.0.36-to-v9.0.37-codex
swift scripts/checkUpstreamTestParity.swift --upstream-ref v9.0.37
swift scripts/checkUpstreamAPIParity.swift --upstream-ref v9.0.37
swift scripts/updateMetadata.swift v9.0.37 --output-root .build/metadata-update/v9.0.37-codex
swift scripts/updateProjectVersions.swift 1.7.7
swift scripts/checkVersionConsistency.swift
scripts/testGeocodingMetadataUpdater.sh
swift test
LC_ALL=ko_KR.UTF-8 LANG=ko_KR.UTF-8 swift test
swift build -c release
swift scripts/testXcodeSchemes.swift --destination 'id=C768167E-DCA0-42C0-A2C0-813F6189F418' --derived-data-root /tmp/libphonenumber-1.7.7-xcode
swift scripts/publishPodspecs.swift --lint
swift scripts/checkMetadataFreshness.swift --current-ref v9.0.37 --output .build/metadata-freshness-v9.0.37-final-codex --fail-on-update
swift scripts/checkUpstreamTestParity.swift --upstream-ref master
swift scripts/checkUpstreamAPIParity.swift --upstream-ref master
swift scripts/checkUpstreamSourceDrift.swift --current-ref v9.0.37 --upstream-ref master --output .build/upstream-source-drift-v9.0.37-to-master-codex
git diff --check
```

### Results

- Scheduled Upstream Drift run `31860137368` detected Google libphonenumber `v9.0.37` as the latest upstream tag; only the metadata freshness job failed, while source drift and Google master parity jobs passed.
- Main phone-number and short-number metadata changed; testing metadata was unchanged.
- Metadata generation resolved `v9.0.37` to peeled commit `b232be1f0da1cc5399359b0c51a25cb3c0c76820`.
- Carrier metadata increased from 31,033 to 31,038 prefix rows and retained 107 mobile portable regions.
- Geocoding databases were unchanged after regeneration.
- Timezone metadata decreased from 3,295 to 3,290 prefix rows and from 286 to 285 unique timezone IDs.
- Upstream `v9.0.37` and Google `master` JS test parity passed with 173 upstream JS tests and 181 local Objective-C tests.
- Upstream `v9.0.37` and Google `master` JS API parity passed with 66 upstream public prototype methods and 93 local Objective-C public selectors.
- Google `master` source/resource drift check passed with zero changed tracked files after `v9.0.37`.
- Version consistency passed for `1.7.7`.
