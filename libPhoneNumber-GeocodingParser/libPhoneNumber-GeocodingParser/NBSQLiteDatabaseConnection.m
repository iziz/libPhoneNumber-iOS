...
static NSString *const createTablePreparedStatement =
    @"CREATE TABLE IF NOT EXISTS geocodingPairs%@ ("
    "ID INTEGER PRIMARY KEY AUTOINCREMENT, "
    "NATIONALNUMBER TEXT, "
    "DESCRIPTION TEXT)";
static NSString *const createIndexStatement = @"CREATE INDEX IF NOT EXISTS nationalNumberIndex ON geocodingPairs%@(NATIONALNUMBER)";
...
- (instancetype)initWithCountryCode:(NSString *)countryCode
  ...
  self = [super init];
  if (self != nil) {
    NSString *dbFileName = [NSString stringWithFormat:@"%@.db", language];
    NSString *databasePath = [destinationPath.path stringByAppendingPathComponent:dbFileName];
    self.sqliteDatabaseCode = sqlite3_open([databasePath UTF8String], &_DB);

    if (_sqliteDatabaseCode == SQLITE_OK) {
      [self createTable:countryCode];
      const char *formattedPreparedStatement = [[NSString stringWithFormat:insertPreparedStatement, countryCode] UTF8String];
      sqlite3_prepare_v2(self.DB, formattedPreparedStatement, -1, &_insertStatement, NULL);
    } else {
      NSLog(@"Cannot open database at desired location: %@\n"
            "SQLite3 Error Message: %s (%d)",
            destinationPath, sqlite3_errstr(self.sqliteDatabaseCode), self.sqliteDatabaseCode);
    }
  }
  return self;
}
...
- (void)addEntryToDB:(NSString *)phoneNumber
  ...
  if (self.sqliteDatabaseCode != SQLITE_OK) {
    NSLog(@"Cannot open database. Failed to add entry. \n"
          "SQLite3 Error Message: %s",
          sqlite3_errstr(self.sqliteDatabaseCode));
    return;
  }
  ...
  sqlite3_step(self.insertStatement);
}
...
- (void)createTable:(NSString *)countryCode {
  ...
  if (sqlite3_exec(self.DB, sqliteCreateTableStatement, NULL, NULL, &sqliteErrorMessage) != SQLITE_OK) {
  ...
  if (sqlite3_exec(self.DB, sqlCreateIndexStatement, NULL, NULL, &sqliteErrorMessage) != SQLITE_OK) {
...
- (int)resetInsertStatement {
  ...
  int sqlResultCode = sqlite3_reset(self.insertStatement);
  if (sqlResultCode != SQLITE_OK) {
    return sqlResultCode;
  } else {
    return sqlite3_clear_bindings(self.insertStatement);
  }
}
...
- (int)createInsertStatement:(NSString *)phoneNumber withDescription:(NSString *)description {
  ...
  if (sqliteResultCode != SQLITE_OK) {
    NSLog(@"SQLite3 error occurred when resetting and clearing bindings in insert statement: %s", sqlite3_errstr(sqliteResultCode));
